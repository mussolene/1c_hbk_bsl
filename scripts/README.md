...
- **Расширение VS Code:** `vsce` требует literal `version` в **`vscode-extension/package.json`**; корень **`vscode-extension/package-lock.json`** должен совпадать. Перед релизной или локальной VSIX-сборкой обновляйте release metadata скриптом **`scripts/sync_version.py`** (или **`make sync-version`**). Скрипт берёт явный release env (`SETUPTOOLS_SCM_PRETEND_VERSION`, `GITHUB_REF_NAME`, `GITHUB_REF`) или последний тег `v*`, но не dirty/dev версию рабочей копии.
...
Локально без тега на коммите после последнего тега setuptools-scm может выдать версию вида `X.Y.Z.devN+gHASH` — это нормально для Python-разработки. В release metadata такая строка не записывается: `scripts/sync_version.py` синхронизирует файлы по стабильному release tag.
