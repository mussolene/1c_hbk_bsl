...
1. **`bslAnalyzer.serverPath`** — явный путь к исполняемому файлу (удобно для разработки и отладки).
...
3. **`PATH`** — команда `bsl-analyzer` (или `bsl-analyzer.exe` в Windows).
4. **Кэш после скачивания** — глобальное хранилище расширения.
5. **Предложение скачать** с GitHub Releases — если ничего не найдено (см. константу тега релиза в `src/extension.ts`).

Имеет смысл держать **`serverPath`** пустым/`bsl-analyzer` для пользователей, которые ставят готовый VSIX с бинарником или кладут бинарник в `PATH`.
