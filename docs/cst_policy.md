...
Повышенный приоритет у правил, где построчный regex даёт ложные срабатывания. Кандидаты: вызовы в циклах (`BSL033`) и другие BSLLS-совместимые правила. Правило **BSL014** может оставаться построчным.
...
|-----|------------------------|----------|------------|
| BSL004 | да | нет | CST-only `if` / `elseif` / `else`; comment-only тело пустое, `Исключение` не диагностируется по BSLLS-семантике |
| BSL009 | да | regex | |
