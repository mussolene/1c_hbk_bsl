...
После последующей parity-итерации целевые большие oracle-корпуса закрыты до `exact_match=true`: нет `only_ours`, `only_bslls`, message, severity и anchor mismatch. Оставшийся продуктовый фокус переносится с удаления известных диагностических gap-блоков на:

- удержание BSLLS-parity как release-gate на выбранных корпусах;
- расширение корпуса oracle-проверок малыми, воспроизводимыми срезами;
- LSP/extension stability: запуск, индексация, latency, Docker/local binary parity;
- документацию единого BSLLS-совместимого профиля без legacy/compat режимов.

Разделы ниже оставлены как история исходной оценки и не должны использоваться как актуальный список известных gap-блоков.
