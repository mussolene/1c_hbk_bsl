...
def extend_runtime_tail_rule_tasks(
    ...
    *,
    ...
    if engine._rule_enabled("BSL178"):
        rule_tasks.append(
            ("BSL178", lambda: engine._rule_bsl178_deprecated_methods_8317(path, lines, tree))
        )
    if engine._rule_enabled("BSL183"):
        rule_tasks.append(
            ("BSL183", lambda: engine._rule_bsl183_execute_external_code(path, lines))
        )
    if engine._rule_enabled("BSL186"):
        rule_tasks.append(("BSL186", lambda: engine._rule_bsl186_extra_commas(path, lines)))
    ...
    if engine._rule_enabled("BSL208") or engine._rule_enabled("BSL256"):
        ...
        def task_bsl208_bsl256() -> list[object]:
            out = engine._rule_bsl208_bsl256_latin_cyrillic_and_typo(path, lines, procs)
